...
Proof. 
  ...
  Case "m = S m'".
    rewrite <- IHm'. simpl. rewrite -> IHn'. rewrite -> IHn'. reflexivity. Qed.


Theorem plus_comm : forall n m : nat,
  n + m = m + n.
Proof.
  intros n m. induction n as [|n']. induction m as [|m'].
  Case "n = 0".
   simpl. reflexivity.
  Case "m = S m".
...
Theorem plus_assoc : forall n m p : nat,
  n + (m + p) = (n + m) + p.
Proof.
  intros n m p. induction n as [|n']. induction m as [|m']. induction p as [|p'].
  Case "n = 0".
   simpl. reflexivity.
  Case "p = S p'".
   simpl. reflexivity.
...
(* Destruct displays the possible values for the variable and forces the
    programmer to prove each case. Induction forces the computer to 
    recurse through the possible values of the variable (usually a natural
    number). Destruct is best when the possiblities are limited, induction 
    is best for when the possiblilities are limitless. *)
...
Proof.
  intros n m p H. induction p as [|p']. induction m as [|m']. induction n as [|n'].
  Case "p = 0".
   simpl. reflexivity.
  Case "n = S n'".
