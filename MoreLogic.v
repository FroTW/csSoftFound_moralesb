...
Proof. 
  intros X P H. unfold not. intros H0. induction H0 as [x H0].
  apply H0. apply H.
Qed.
(** [] *)
...
Proof.
   ...
   Case "rightSide". intros. inversion H as [x [H0 | H1]].
      left. exists x. apply H0.
      right. exists x. apply H1.
   Case "leftSide". intros. inversion H as [[x H2] | [x H3]].
      exists x. left. apply H2.
...
Inductive all (X : Type) (P : X -> Prop) : list X -> Prop :=
  |all_base : all X P[]
  |all_cons : forall(h:X) (r: list X), P h -> all X P (h::r)
  .
...
Theorem all_forallb : forall X(l : list X) (test : X -> bool),
   forallb test l = true <-> all X(fun x => test x = true) l.
Proof.
   intros. split. intros.
   Case "forall implies all".
     simpl in H.
     induction l as [|h r].
       SCase "1 = nil".
         apply all_base.
...
Inductive nostutter:  list nat -> Prop :=
 ...
 |nostutter1 : forall x, nostutter [x]
